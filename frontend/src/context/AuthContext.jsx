...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    if (token) {
      api.get('/auth/me')
        ...
        .catch(() => { localStorage.removeItem('ct_token'); setToken(null) })
  ...
  const login = async (email, password) => {
    ...
    localStorage.setItem('ct_token', r.data.access_token)
  ...
  // Placeholder - Google OAuth will be added later
  const loginWithGoogle = async () => {
    throw new Error('Google login coming soon')
  }

  return (
    <AuthContext.Provider value={{ user, token, loading, login, signup, logout, loginWithGoogle }}>
      {children}
